...
int read_bigcharset_callback(char *rest)
{
	...
	if (!Strcasecmp(opt,"chars") && charheight)
	{
		...
		opt = chop(&rest);
		set_mallocdoer(read_bigcharset_callback);
		newchar = (BigC*)Malloc(sizeof(BigC) + strlen(opt));
		newchar->width = 0;
		newchar->data = NULL;
...
void do_bigsay(COMMAND_ARGS)
{
	...
	{
		to_user(from,"error loading font file: %s\n",output);
		return;
	}
...
void do_random_msg(COMMAND_ARGS)
{
	...
	if (CurrentCmd->name == C_PICKUP)
		filename = RANDPICKUPFILE;
	else
	if (CurrentCmd->name == C_INSULT)
		filename = RANDINSULTFILE;
	else
	/* if (CurrentCmd->name == C_RSAY) */
		filename = RANDSAYFILE;

	if (rest)
	{
		to = chop(&rest);
		if (ischannel(to) && (get_authlevel(from,to) < cmdaccess))
			return;
	}
	else
	if (!ischannel(to))
...
void do_8ball(COMMAND_ARGS)
{
	...
	{
		to_user(from,ERR_FILEOPEN,RAND8BALLFILE);
		return;
	}

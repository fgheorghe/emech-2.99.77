...
void delete_client(Client *client)
{
	...
	if (client->user)
	{
		...
		send_global("[%s] %s[%i] has disconnected",current->nick,client->user->name,(int)client->user->access);
	}
#ifdef DCC_FILE
	if (client->fileno >= 0)
		close(client->fileno);
...
void partyline_broadcast(Client *from, char *format, char *rest)
{
	...
	for(bot=botlist;bot;bot=bot->next)
	{
		...
		{
			if (0 == (client->flags & DCC_ACTIVE))
				continue;
			if (client == from && !client->user->echo)
				continue;
...
void dcc_banner(Client *client)
{
	...
	sprintf(tmp,"[%s] %s[%i] has connected",
		current->nick,client->user->name,(int)client->user->access);
	...
	send_global(tmp);
	if (client->user->access == OWNERLEVEL)
	{
		CurrentDCC = client;
...
int dcc_sendfile(char *target, char *filename)
{
	...
	Client	*client;
	int	s,f,sz;
	char	tempfile[strlen(filename)+strlen(DCC_PUBLICFILES)+2];
	...
	client = (Client*)Calloc(sizeof(Client) + strlen(filename) + strlen(target));
...
void dcc_pushfile(Client *client, ulong where)
{
	char	bufblock[4096];
	int	fp,sz;

	fp = lseek(client->fileno,0,SEEK_CUR);
	where += 16384;
	while(fp<where)
	{
		sz = read(client->fileno,bufblock,4096);
		if (sz == 0)
		{
			return;
		}
		write(client->sock,bufblock,sz);
		fp = lseek(client->fileno,0,SEEK_CUR);
	}
}
...
void parse_dcc(Client *client)
{
	...
	if (client->flags & DCC_RECV)
	{
		...
		{
#ifdef DEBUG
			debug("(parse_dcc) DCC file recv `%s' completed in %i seconds (%i bytes)\n",
	...
	if (client->flags & DCC_SEND)
	{
		...
		s = client->inputcount;
		oc = read(client->sock,(client->sd+s),(4-s));
		if (oc < 0)
		{
			client->flags = DCC_DELETE;
			return;
		...
		if (oc == 4)
		{
			where = ((uchar)client->sd[0] << 24) | ((uchar)client->sd[1] << 16) | ((uchar)client->sd[2] << 8) | (uchar)client->sd[3];
			client->inputcount = 0;
...
#endif /* DCC_FILE */
	ptr = sockread(client->sock,client->sd,text);
	oc = errno;
	if (ptr)
	{
		/*
...
void ctcp_dcc(char *from, char *to, char *rest)
{
	...
	if (!Strcasecmp(port,"SEND"))
	{
		...
		{
#ifdef DEBUG
			debug("(ctcp_dcc) filename `%s' contains slashes\n",filename);
#endif /* DEBUG */
			return;
		}
		/*
		...
		if (1)
		{
			char tempname[strlen(filename)+strlen(DCC_PUBLICINCOMING)+1];
			...
			client = (Client*)Malloc(sizeof(Client) + strlen(filename) + strlen(from));
			client->fileno = f;
...
#endif /* DCC_FILE */
	if ((x >= MINUSERLEVEL) && (x < BOTLEVEL) && !Strcasecmp(port,"CHAT"))
	{
		if ((user = get_authuser(from,NULL)) == NULL)
		{
...
void ctcp_finger(char *from, char *to, char *rest)
{
	 ...
	 */
	if (current->spy.status)
	{
		maxul = get_maxuserlevel(from);
	...
	send_ctcp_reply(CurrentNick,"FINGER %s",(reply) ? reply : EXFINGER);
}
...
void ctcp_version(char *from, char *to, char *rest)
{
	...
	send_ctcp_reply(CurrentNick,"VERSION %s",(reply) ? reply : EXVERSION);
}
...
void whom_printbot(char *from, BotInfo *binfo, char *stt)
{
	...
	if (binfo->nuh)
	{
		uaccess = get_maxuserlevel(binfo->nuh);
		if (uaccess == BOTLEVEL)
			us = "b200";
		else
		if (uaccess)
	...
	to_user(from,TEXT_WHOMBOTLINE,(binfo->nuh) ? nickcpy(NULL,binfo->nuh) : "???",us,
		(binfo->server) ? binfo->server : "???",binfo->guid);
}
...
void do_whom(COMMAND_ARGS)
{
	...
	for(bot=botlist;bot;bot=bot->next)
	{
		...
		to_user(from,TEXT_WHOMSELFLINE,sps,(bot == current) ? "(me)" : "b200",stt);
		for(client=bot->clientlist;client;client=client->next)
		{
			m = (now - client->lasttime) / 60;
			s = (now - client->lasttime) % 60;
			sprintf(sps,"u%i",(int)client->user->access);
			to_user(from,TEXT_WHOMUSERLINE,
				client->user->name,sps,"DCC",m,s);
		}
	}
#ifdef BOTNET
	for(bn=botnetlist;bn;bn=bn->next)
	{
		if (bn->status != BN_LINKED)
			continue;
		if ((binfo = bn->rbot))
			whom_printbot(from,bn->rbot,stt);
...
void do_send(COMMAND_ARGS)
{
	...
	if (dcc_freeslots(get_maxuserlevel(from)) < 1)
	{
		if (ischannel(to))
		{
		...
		to_user(from,"Unable to send: No free DCC slots");
		return;
	}
	sz = dcc_sendfile(target,filename);
	if (sz < 0)
	{
		to_user(from,"Unable to locate file: %s",filename);
		return;
	}
	if (target == CurrentNick)
	{
		to_user(from,"Sending %s (%i bytes)",filename,sz);
	}
	else
	{
		if (ischannel(to))
		...
		to_user(from,"Sending %s (%i bytes) to %s",filename,sz,target);
	}
}

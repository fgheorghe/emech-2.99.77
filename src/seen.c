...
int read_seenlist(void)
{
	...
	if (!seenfile)
		return(FALSE);
	if ((in = open(seenfile,O_RDONLY)) < 0)
		return(FALSE);
...
step_two:
	...
	seen = (Seen*)Malloc(sizeof(Seen) + strlen(nick) + strlen(userhost) +
		((pa) ? strlen(pa) : 0) + ((pb) ? strlen(pb) : 0));

	seen->next = *pp;
	*pp = seen;
	seen->when = when;
	seen->t = t;
	seen->pa = seen->pb = NULL;
...
void do_seen(COMMAND_ARGS)
{
	...
	/*
	 *  seen can be a public command, make sure it doesnt flood the bot off
	 */
	if (!mul)
	{
		for(d=0;d<SEEN_SLOTS;d++)
		{
			if (seen_slot[d] < now)
			{
				seen_slot[d] = now + SEEN_TIMEOUT;
#ifdef DEBUG
				debug("(do_seen) using slot %i\n",d);
#endif /* DEBUG */
				goto slot_taken;
			}
		}
#ifdef DEBUG
		debug("(do_seen) no free slots, ignoring command\n");
#endif /* DEBUG */
		return;
	}
...
slot_taken:
	if (!rest || !*rest)
	{
		if (mul) to_user(from,"Who do you want me look for?");
		return;
	}
	...
	if (!isnick(n))
	{
		if (mul) to_user(from,"invalid nick");
		return;
	}
	...
	{
		/*
		 *  this loop doesnt execute very often, so we can afford to
	...
	if (ischannel(to))
		sendprivmsg(chan,fmt,n,u,c1,c2,c3);
	else
		to_user(from,fmt,n,u,c1,c2,c3);
}

...
#define TEXT_ASK_HANDLE		INTERNAL_SOURCE " PRIVMSG %s :Please enter your handle\n"
#define TEXT_ASK_PASSWORD	INTERNAL_SOURCE " PRIVMSG %s :Please enter your password\n"
...
void process_bounce(void)
{
	...
	while((irc = *pp))
	{
		if (irc->status == BNC_DEAD || ((now - irc->active) > 60))
		{
#ifdef DEBUG
